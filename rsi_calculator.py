...
class RSICalculator(threading.Thread):
    ...
    def _handle_calculate_rsi_requested(self, event: CalculateRSIRequested):
        # MODIFICATION : Désérialiser la chaîne JSON pour reconstruire la Series.
        prices_series = None
        if event.prices_series_json:
            try:
                # 'typ="series"' est important pour que pandas reconstruise une Series.
                prices_series = pd.read_json(event.prices_series_json, orient="split", typ="series")
                # Reconvertir l'index en datetime.
